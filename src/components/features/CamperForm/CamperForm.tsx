...
const STATUS_OPTIONS = Object.entries(CAMPER_STATUS_LABELS) as [string, string][];

function StatusSelect() {
  const { register, formState: { errors } } = useFormContext<ICamperData>();
...
export default function CamperForm({ camper, onSuccess, onCancel }: CamperFormProps) {
  ...
  return (
    <Form methods={methods} onSubmit={handleSubmit} variant="full" className="gap-4">
      ...
      <Input name="imageUrl" label="URL зображення" placeholder="https://..." />
